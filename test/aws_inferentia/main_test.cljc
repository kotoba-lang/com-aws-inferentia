(ns aws_inferentia.main-test
  "Contract + behavioral test for the aws_inferentia-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "aws_inferentia-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'aws_inferentia.main-test)]
