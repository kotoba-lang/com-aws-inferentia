(ns aws_inferentia.main
  "Kotodama WASM entrypoint for the AWS Inferentia clean-room actor (L4) — Clojure port.
  ...
  facts (`aws_inferentia.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "aws_inferentia")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"aws_inferentia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "aws_inferentia-compat" :tier tier :entities entities} 200])
...
(defn start! [] :aws_inferentia-compat/ready)
